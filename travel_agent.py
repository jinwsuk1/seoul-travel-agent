...
def local_multimodal_tool(restaurant_name, region):
    print(f"   🖼️ [멀티모달 도구] Qdrant DB 'seoul_images' 컬렉션에서 '{restaurant_name}' 이미지 매핑 검색 중...")
    
    image_url = ""
    
    try:
        # 💡 핵심 로직: Qdrant DB에 직접 접근하여 상호명(name)으로 메타데이터 검색!
        ...
        # DB에서 매칭되는 이미지를 찾은 경우
        if records:
            # payload(메타데이터) 안에 저장해둔 이미지 경로를 가져옵니다.
            image_url = str(records[0].payload.get("image_path")).replace("\\", "/")
        else:
            # DB에 없는 3만 5천 개의 식당을 위한 안전망 (더미 이미지)
...
def generator_agent(query, search_results, region):
    ...
    * 🗺️ [카카오맵에서 상세 정보 확인하기]([참고 정보에 제공된 카카오맵 URL])
